                ...
                struct gps_location __user *, loc)
{
    ...
    if (file_ind->i_op == NULL ||
        ...
        printk("if the file is in ext3 failure\n");
        path_put(&s_kpath);
        kfree(s_kpathname);
        return -ENODEV;
    }
...
void get_gps_data(struct gps_kdata *data)
{
    ...
    /* @lfred: use printk to check if we are reallt running */
    printk("[HW6] get_gps_data: current time: %ld\n", curr_age);
}
